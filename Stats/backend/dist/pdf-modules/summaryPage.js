...
function calculateCategoryAverages(data, category, role = 'docentes') {
...
const generateSummaryPage = (doc, school) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const drawStackedBar = (label, values, y) => {
        // Draw label
        doc.fontSize(10)
            .font('Helvetica')
            .fillColor('black')
        ...
        if (values.S === 0 && values.A === 0 && values.N === 0) {
            ...
            doc.fontSize(9)
                ...
                .text('Sin información', sectionStartX + stackedBarLabelWidth + (stackedBarWidth / 2) - 40, y + 4, {
                width: 80,
                align: 'center'
            });
            return;
        ...
        if (actualTotal === 0) {
            ...
            doc.fontSize(9)
                .fillColor('#666666') // Dark gray text
                .text('Error de datos', sectionStartX + stackedBarLabelWidth + (stackedBarWidth / 2) - 40, y + 4, {
                width: 80,
                align: 'center'
            });
            return;
        ...
        if (sum > 100) {
            ...
            if (max === normalizedValues.S)
                normalizedValues.S -= (sum - 100);
            else if (max === normalizedValues.A)
                normalizedValues.A -= (sum - 100);
            else
                normalizedValues.N -= (sum - 100);
        }
        else if (sum < 100) {
            // Add remainder to the largest value
            const max = Math.max(normalizedValues.S, normalizedValues.A, normalizedValues.N);
            if (max === normalizedValues.S)
                normalizedValues.S += (100 - sum);
            else if (max === normalizedValues.A)
                normalizedValues.A += (100 - sum);
            else
                normalizedValues.N += (100 - sum);
        }
        ...
        if (values.S > 0) {
            doc.fillColor('white')
                .fontSize(8)
                .text(`${values.S}%`, currentX + (sWidth / 2) - 8, y + (barHeight / 2), { baseline: 'middle' });
        }
        currentX += sWidth;
        ...
        if (values.A > 0) {
            doc.fillColor('black')
                .fontSize(8)
                .text(`${values.A}%`, currentX + (aWidth / 2) - 8, y + (barHeight / 2), { baseline: 'middle' });
        }
        currentX += aWidth;
        // Draw N segment (Red)
        ...
        if (values.N > 0) {
            doc.fillColor('white')
                .fontSize(8)
                .text(`${values.N}%`, currentX + (nWidth / 2) - 8, y + (barHeight / 2), { baseline: 'middle' });
        }
    };
    ...
    doc.fontSize(10)
        ...
        .text('Lo ideal sería que, en los tres componentes, la percepción de cada uno de los actores fuera lo más positiva posible. Identifique en cuáles actores y componentes la percepción negativa es mayor.', instructionBoxX + 10, instructionBoxY + 12, // Approximately vertically centered (40px height / 2 = 20px, minus half text height ~8px)
    {
