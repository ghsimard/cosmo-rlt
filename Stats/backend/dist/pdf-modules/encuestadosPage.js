...
exports.generateEncuestadosPage = void 0;
...
function getEstudiantesCount(school) {
    return __awaiter(this, void 0, void 0, function* () {
        var _a;
        try {
            const query = `
...
function getAcudientesCount(school) {
    return __awaiter(this, void 0, void 0, function* () {
        var _a;
        try {
            const query = `
...
function getScheduleDistribution(school) {
    ...
    `;
            ...
            const chartData = result.rows.map(row => {
                ...
                return {
                    label: ((_a = scheduleMapping[row.schedule]) === null || _a === void 0 ? void 0 : _a.label) || row.schedule, // Simple label without percentage
...
function getScheduleDistributionForEstudiantes(school) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const query = `
      SELECT 
...
function getGradosEstudiantesDistribution(school) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const query = `
      WITH grade_data AS (
...
function getYearsDistribution(school) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
    ...
    `;
            ...
            const dbCategories = ['Menos de 1', '1', '2', '3', '4', '5', 'Más de 5'];
...
function getFeedbackDistribution(school) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
      ...
      WHERE institucion_educativa = $1;
    `;
            const checkResult = yield db_1.pool.query(checkQuery, [school]);
    ...
    `;
            ...
            const { rows } = yield db_1.pool.query(query, [school]);
...
function getYearsDistributionForEstudiantes(school) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
...
const generateEncuestadosPage = (doc, school) => __awaiter(void 0, void 0, void 0, function* () {
    // Add a new page with header
    doc.addPage();
    (0, pdfUtils_1.addHeader)(doc);
    const startX = 75;
    ...
    (0, pdfUtils_1.drawPieChart)(doc, gradesData, startX + chartWidth / 4, // Moved more to the left (changed from chartWidth/2.5)
    startChartY + chartHeight / 2, chartWidth / 4, '¿En qué grados tiene clases?', true, // First chart
    ...
    (0, pdfUtils_1.drawPieChart)(doc, scheduleData, separatorX + chartWidth / 3, // Adjusted position to make room for right legend
    startChartY + chartHeight / 2, chartWidth / 4, '¿En qué jornada tiene clases?', false, // Not first chart
    false, // Place legend on the right (not below)
    false // Don't use multi-line legend
    );
    ...
    const yearsData = yield getYearsDistribution(school);
    ...
    (0, pdfUtils_1.drawBarChart)(doc, yearsData, yearsChartX, yearsChartY, yearsChartWidth, yearsChartHeight, '¿Cuántos años lleva en la IE?', true // isHorizontal
    );
    // Draw second bar chart (Feedback)
    console.log('About to draw feedback chart');
    const feedbackData = yield getFeedbackDistribution(school);
    console.log('Drawing feedback chart with data:', feedbackData);
    // Draw the chart with adjusted padding
    (0, pdfUtils_1.drawBarChart)(doc, feedbackData, startX + chartWidth + chartMargin, horizontalSeparatorY + 5, // Position it just below the horizontal separator
    chartWidth, chartHeight, 'Usted recibe retroalimentación de', true // isHorizontal
    );
    ...
    doc.save()
        ...
        .fill()
        .restore();
    const estudiantesCount = yield getEstudiantesCount(school);
    doc.fontSize(12)
        .font('Helvetica-Bold')
        .fillColor('white') // White text for contrast
    ...
    const estudiantesGradesData = yield getGradesDistributionForEstudiantes(school);
    (0, pdfUtils_1.drawPieChart)(doc, estudiantesGradesData, startX + chartWidth / 3, estudiantesChartY + chartHeight / 2, chartWidth / 4, '¿En qué grado te encuentras?', true, true, // Place legend below
    ...
    const estudiantesScheduleData = yield getScheduleDistributionForEstudiantes(school);
    (0, pdfUtils_1.drawPieChart)(doc, estudiantesScheduleData, startX + chartWidth * 1.0 + chartMargin, // Reduced from 1.2 to 1.0 to move it more left
    estudiantesChartY + chartHeight / 2, chartWidth / 4, '¿En qué jornada tiene clases?', false, true, // Place legend below
    ...
    const yearsDataRight = yield getYearsDistributionForEstudiantes(school);
    (0, pdfUtils_1.drawBarChart)(doc, yearsDataRight, verticalSeparatorX + 20, // Keep 20 points padding from separator
    estudiantesChartY, // Changed from -8 to +15 to move chart lower
    chartWidth / 1.5, chartHeight, '¿Cuántos años lleva en la IE?', true // isHorizontal
    );
    ...
    const acudientesCount = yield getAcudientesCount(school);
    doc.fontSize(12)
        .font('Helvetica-Bold')
        .fillColor('white') // White text for contrast
    ...
    const stackedBarData = yield getGradosEstudiantesDistribution(school);
    ...
    stackedBarData.forEach((item, index) => {
        ...
        doc.fillColor('black')
            ...
            .text(item.label, legendItemX + 12, rowY, { width: legendItemWidth - 12 });
    });
    ...
    doc.fontSize(10)
        ...
        .fillColor('#FF0000') // Red color
        .text('Analice la composición de los distintos grupos encuestados y tenga encuenta que esta muestra no representa la totalidad de su IE.', textStartX + notePadding, noteY + 8, {
        width: noteTextWidth - (notePadding * 2), // Use noteTextWidth here
        align: 'center'
    });
});
exports.generateEncuestadosPage = generateEncuestadosPage;
function drawPieChart(doc, data, centerX, centerY, radius, title, showLegend = false, placeLegendBelow = false, multiLineLegend = false) {
    try {
        ...
        data.forEach((segment, index) => {
            ...
            if (percentage > 5) { // Only show percentage if segment is > 5%
                ...
                doc.save()
                    .fillColor('white')
                    .fontSize(8)
                    .text(`${percentage}%`, labelX - 10, // Center the percentage text
            ...
            doc.fillColor('black')
                .fontSize(8)
                .text(segment.label, legendX + 15, legendY + 2, {
